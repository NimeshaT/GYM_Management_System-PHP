...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Report on Fitness Income</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (@$rep_type == "D") {
                    $sql = "SELECT invoiceDate,SUM(total) AS mysum FROM tbl_fitness_invoice $where GROUP BY invoiceDate";
                }
                if (@$rep_type == "M") {
                    $sql = "SELECT MONTH(invoiceDate)AS Month,SUM(total) AS mysum FROM tbl_fitness_invoice $where GROUP BY MONTH(invoiceDate)";
                }
                if (@$rep_type == "Y") {
                    $sql = "SELECT YEAR(invoiceDate)AS Year,SUM(total) AS mysum FROM tbl_fitness_invoice $where GROUP BY YEAR(invoiceDate)";
                }
                if (@$rep_type == "W") {
                    $sql = "SELECT WEEK(invoiceDate)AS Week,SUM(total) AS mysum FROM tbl_fitness_invoice $where GROUP BY WEEK(invoiceDate)";
                }
                $result = $db->query($sql);
                $total = 0;
                ?>
