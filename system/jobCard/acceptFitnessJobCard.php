            ...
            <div class="row">
                <div class="card">
                    <div class="card-header bg-info">
                        <h3 class="card-title">Instructor Fitness Job Card</h3>
                    </div>
                    <div class="card-body">
                        <?php
                        extract($_POST);
...
//                            $Stid = $Stid == '7' ? '8' : '7';
                            ...
                            $sql = "UPDATE tbl_fitness_job_card SET statusId='$Stid' WHERE fitnessJobCardId='$Sjid'";
                            $db->query($sql);
                        }
...
//        INNER JOIN tbl_members ON tbl_appointments.memberId = tbl_members.memberId INNER JOIN tbl_time_slots ON tbl_appointments.slotId=tbl_time_slots.slotId WHERE instructorId='" . $_SESSION['INSTRUCTORID'] . "' $where ORDER BY jobCardId DESC";
                        $sql1="SELECT tbl_fitness_job_card.statusId AS jobCardStatusId, tbl_status.statusName, tbl_fitness_job_card.fitnessJobCardId, 
                tbl_bookings.bookingId, tbl_bookings.bookingDate, tbl_bookings.appointmentTypeId, 
                tbl_fitness.fitnessId, tbl_fitness.fitnessName, 
         ...
         FROM tbl_fitness_job_card 
         INNER JOIN tbl_status ON tbl_fitness_job_card.statusId = tbl_status.statusId 
         INNER JOIN tbl_fitness ON tbl_fitness_job_card.fitnessId = tbl_fitness.fitnessId 
         INNER JOIN tbl_bookings ON tbl_fitness_job_card.bookingId = tbl_bookings.bookingId 
         INNER JOIN tbl_members ON tbl_bookings.memberId = tbl_members.memberId 
         INNER JOIN tbl_time_slots ON tbl_bookings.slotId = tbl_time_slots.slotId 
         WHERE tbl_fitness_job_card.instructorId = '" . $_SESSION['INSTRUCTORID'] . "' $where ORDER BY tbl_fitness_job_card.fitnessJobCardId DESC";
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                                                        ...
                                                        <input type="hidden" name="Sjid" value="<?php echo $row['fitnessJobCardId'] ?>">
